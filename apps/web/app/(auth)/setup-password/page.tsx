...
import { setupPasswordSchema } from "@isysocial/shared";
...
type SetupForm = z.infer<typeof setupPasswordSchema>;
...
function SetupPasswordContent() {
  ...
  const setupPassword = trpc.auth.setupPassword.useMutation({
  ...
  const onSubmit = async (data: SetupForm) => {
    await setupPassword.mutateAsync(data);
  };
      ...
      <div className="min-h-screen flex items-center justify-center p-4">
        <Card className="max-w-md w-full">
          <CardContent className="pt-6 text-center">
            <p className="text-destructive">Enlace de invitación inválido.</p>
          </CardContent>
        </Card>
      </div>
    );
      ...
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-background to-secondary/30 p-4">
        <Card className="max-w-md w-full shadow-soft text-center">
          <CardContent className="pt-8 pb-8 space-y-4">
            ...
            <h2 className="text-xl font-semibold">¡Contraseña configurada!</h2>
        ...
        <Card className="shadow-soft border-0 bg-card/95 backdrop-blur-sm">
          <CardHeader className="pb-4">
            <CardTitle className="text-xl">
              {tokenLoading ? "Verificando enlace..." : `Bienvenido${tokenData?.userName ? `, ${tokenData.userName}` : ""}`}
            </CardTitle>
            <CardDescription>
