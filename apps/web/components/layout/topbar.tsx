...
export function Topbar({ title }: { title: string }) {
  ...
  const { data: notifsData } = trpc.notifications.list.useQuery(
    { limit: 5 },
    { enabled: open }
  );
  const recentNotifs = notifsData?.notifications;
  ...
  // Auto-mark all as read when dropdown opens and there are unread notifications
  useEffect(() => {
