...
export function InstagramStoryMockup({
  ...
  return (
    <div className={cn(
      ...
      {/* iPhone Frame - Outer bezel */}
      <div className="absolute inset-0 bg-black rounded-[40px] shadow-2xl p-3">
        {/* Mute button */}
        <div className="absolute left-0 top-24 w-1 h-12 bg-gray-400 rounded-r-lg" />
        
        {/* Volume buttons */}
        <div className="absolute left-0 top-40 w-1 h-8 bg-gray-400 rounded-r-lg" />
        <div className="absolute left-0 top-52 w-1 h-8 bg-gray-400 rounded-r-lg" />

        {/* Speaker cutout */}
        <div className="absolute top-3 left-1/2 -translate-x-1/2 w-32 h-2 bg-black rounded-b-3xl z-30" />

        {/* Inner display */}
        <div className="relative w-full h-full bg-zinc-900 rounded-[35px] overflow-hidden flex flex-col">
          {/* Dynamic Island / Notch */}
          <div className="absolute top-0 left-1/2 -translate-x-1/2 w-40 h-7 bg-black rounded-b-3xl z-20 flex items-center justify-center">
            <div className="w-1.5 h-1.5 rounded-full bg-black/40" />
          </div>

          {/* Status bar (behind notch) */}
          <div className="absolute top-2 left-4 right-4 h-5 z-10 flex items-center justify-between text-white text-xs font-semibold">
            <span>9:41</span>
          ...
          {/* Header */}
          <div className="absolute top-12 left-3 right-3 flex items-center gap-2 z-20">
