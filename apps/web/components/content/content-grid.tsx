...
interface ContentGridProps {
  ...
  basePath: string; // e.g., "/admin/contenido", "/editor/contenido", "/cliente/contenido"
  showClient?: boolean;
}

export function ContentGrid({ posts, basePath, showClient = false }: ContentGridProps) {
  return (
      ...
      {posts.map((post) => {
        ...
        return (
          <Link key={post.id} href={`${basePath}/${post.id}`}>
            <Card className={`group hover:shadow-lg transition-all duration-200 cursor-pointer overflow-hidden h-full ${needsAction ? "ring-2 ring-yellow-400/50" : ""}`}>
              {/* Thumbnail */}
              ...
              <CardContent className="p-3 space-y-1.5">
                ...
                {/* Client name (for admin/editor views) */}
